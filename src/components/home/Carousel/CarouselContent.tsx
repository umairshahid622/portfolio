...
const CarouselContent = ({ item }: { item: WorkExperience }) => {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-center px-4 lg:px-14">
      <div className="space-y-2">
        <h1 className="leading-none font-extrabold text-transparent text-outline">{item.num}</h1>
        <h5>{item.title}</h5>
        <h6 className="text-light-text-muted dark:text-dark-text-muted">{item.companyName}</h6>
        <p>
      ...
      <div className="py-4 px-8 rounded-lg relative overflow-hidden bg-light-card dark:bg-dark-card">
        ...
        <div className="h-1 w-full bg-brand-green absolute bottom-0 left-0"></div>
      </div>
    </div>
  );
};
